...
//
//  HOW TO RUN:
//  1. Upload perceptron_reader.ino to Arduino first
//  2. Open this file in Processing and hit Run (▶)
//  3. Check the console for port list — change ports[0] if needed
//  4. No Arduino? Runs in DEMO MODE automatically
// ─────────────────────────────────────────────────────────
...
void setup() {
  ...
  for (int i = 0; i < history.length; i++) history[i] = 0.5;

  String[] ports = Serial.list();
  println("─── Available serial ports ───");
  for (int i = 0; i < ports.length; i++) println(i + ": " + ports[i]);
  println("Change ports[0] below if the wrong port is selected.");
  println("──────────────────────────────");

  if (ports.length > 0) {
    try {
      port = new Serial(this, ports[6], 9600); // Change index if needed
      port.bufferUntil('\n');
      demo = false;
      println("Connected to: " + ports[6]);
    } catch (Exception e) {
      println("Could not open port — running in demo mode.");
    }
  } else {
...
void serialEvent(Serial p) {
  ...
  // Enforce the 0.45 threshold logic directly in Processing
  fired = (sigma > 0.45) ? 1 : 0;
...
void draw() {
  ...
  firedGlow = lerp(firedGlow, (float)fired, 0.12);
...
// ── Demo: simulates standard non-inverting summer behaviour 
void animateDemo() {
  ...
  // Standard summer: output rises when inputs are active
  sigma = (w1 * x1 + w2 * x2 + b) / 3.0;
  sigma = constrain(sigma, 0, 1);
  fired = (sigma > 0.45) ? 1 : 0;
...
// ── Header ────────────────────────────────────────────────
void drawHeader() {
  noStroke(); fill(12, 9, 2);
  rect(0, 0, width, 36);
  stroke(DIM); strokeWeight(1);
  line(0, 36, width, 36);

  noStroke(); fill(BRIGHT);
  textSize(13); textAlign(LEFT, CENTER);
  text("PERCEPTRON MONITOR  v1.2", 14, 18);

  textAlign(CENTER, CENTER); fill(DIM);
...
void drawNetwork() {
  ...
  // Connections — bias line is always active (always glowing)
  weightLine(ix1, iy1, sx, sy, w1, x1 == 1, "w\u2081=" + nf(w1, 1, 2));
  weightLine(ix2, iy2, sx, sy, w2, x2 == 1, "w\u2082=" + nf(w2, 1, 2));
  weightLine(ibx, iby, sx, sy, b,  true,     "b =" + nf(b, 1, 2));  // bias always on

  // Σ → output line: blue when idle, green when fired
  stroke(lerpColor(BLUE, GREEN, firedGlow));
  strokeWeight(lerp(1.5, 5, firedGlow));
  line(sx + 29, sy, ox - 31, oy);

  inputNode(ix1, iy1, x1, "x\u2081");
  inputNode(ix2, iy2, x2, "x\u2082");
  biasNode(ibx, iby);          // always lit — bias is always connected
  sumNode(sx, sy, sigma);
...
void inputNode(float x, float y, int active, String lbl) {
  ...
  fill(BRIGHT); textSize(12); textAlign(RIGHT, CENTER);
  text(lbl, x - 28, y);

  fill(nc); textSize(10); textAlign(CENTER, TOP);
  text(active == 1 ? "ON" : "OFF", x, y + 27);
}

// ── Bias node — always glowing amber (bias is always on) ──
void biasNode(float x, float y) {
  noStroke();
  for (int r = 46; r > 20; r -= 6) { fill(255, 165, 0, 14); ellipse(x, y, r*2, r*2); }

  strokeWeight(2); stroke(AMBER);
  fill(color(35, 22, 0));
  ellipse(x, y, 46, 46);
  ...
  fill(AMBER); textSize(10); textAlign(RIGHT, CENTER);
  text("bias", x - 28, y);

  fill(AMBER); textSize(10); textAlign(CENTER, TOP);
  text("ON", x, y + 27);
}

// ── Sum node ──────────────────────────────────────────────
void sumNode(float x, float y, float val) {
  // Glow intensity directly maps to voltage/summation
  float glow = val;
  noStroke();
  for (int r = 60; r > 25; r -= 7) { fill(255, 165, 0, int(glow * 22)); ellipse(x, y, r*2, r*2); }

  strokeWeight(2.5); stroke(lerpColor(DIM, AMBER, glow));
  fill(color(int(glow * 35), int(glow * 22), 0));
  ellipse(x, y, 58, 58);
  ...
  fill(DIM); textSize(10); textAlign(CENTER, TOP);
...
void outputNode(float x, float y, int fire) {
  if (fire == 1) {
    noStroke();
    for (int r = 74; r > 30; r -= 8) { fill(57, 255, 20, 10); ellipse(x, y, r*2, r*2); }
  } else {
    noStroke();
    for (int r = 74; r > 30; r -= 8) { fill(0, 170, 255, 8); ellipse(x, y, r*2, r*2); }
  }

  color nc = fire == 1 ? GREEN : BLUE;
  strokeWeight(3); stroke(nc);
  fill(fire == 1 ? color(8, 35, 8) : color(0, 15, 30));
  ellipse(x, y, 62, 62);
  ...
  if (firedGlow > 0.15) {
    fill(GREEN); textSize(13); textAlign(CENTER, TOP);
    text("FIRED!", x, y + 37);
  }
}
...
void weightLine(float x1c, float y1c, float x2c, float y2c, float w, boolean act, String lbl) {
  ...
  stroke(lc); strokeWeight(map(w, 0, 1, 1.0, 4.5));
  noFill();
...
void drawMetrics() {
  ...
  noStroke(); fill(10, 8, 2);
  rect(px - 8, py, pw, ph, 5);
  stroke(DIM); strokeWeight(1); noFill();
  rect(px - 8, py, pw, ph, 5);
  ...
  // Weight bars: green when that input is ON, amber otherwise
  metricBar(px, py + 35,  pw - 20, w1, "w\u2081", x1 == 1 ? GREEN : AMBER);
  metricBar(px, py + 72,  pw - 20, w2, "w\u2082", x2 == 1 ? GREEN : AMBER);
  // Bias bar: always amber (always on)
  metricBar(px, py + 109, pw - 20, b,  "b ",  AMBER);

  text("\u2500\u2500\u2500 SUM \u2500\u2500\u2500\u2500", px, py + 148);
  
  // Sigma bar: shows actual summation value 
  metricBar(px, py + 173, pw - 20, sigma, "\u03A3 ", lerpColor(BLUE, GREEN, firedGlow));

  text("\u2500\u2500 OUTPUT \u2500\u2500\u2500", px, py + 215);
  fill(firedGlow > 0.5 ? GREEN : BLUE);
  textSize(34); textAlign(CENTER, CENTER);
...
void metricBar(float x, float y, float w, float val, String lbl, color col) {
  ...
  rect(x, y, w, 19, 3);
  noStroke(); fill(col);
  rect(x + 1, y + 1, (w - 2) * constrain(val, 0, 1), 17, 2);
  noStroke(); fill(BRIGHT);
...
void drawScope() {
  ...
  // Grid
  stroke(20, 28, 12); strokeWeight(1);
  ...
  // Output fires when sigma rises above this mark.
  float ty = sy + sh - 0.45 * sh;
  stroke(DIM); strokeWeight(1);
  for (int xi = int(sx); xi < int(sx + sw) - 4; xi += 9) line(xi, ty, xi + 5, ty);

  noStroke(); fill(DIM);
  textSize(9); textAlign(LEFT, CENTER);
  text("\u03B8=0.45", sx + 6, ty - 8);  
  textAlign(LEFT, TOP);
...
// ── CRT scanlines ─────────────────────────────────────────
void drawScanlines() {
  stroke(0, 0, 0, 32); strokeWeight(2);
